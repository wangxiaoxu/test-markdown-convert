...
- [x] **2.3 测试Docker环境**
  ...
  - 验证LaTeX可用: docker-compose exec converter xelatex --version
  - **验证**: 所有命令执行成功，Pandoc版本输出为 3.1.11.1
  - **依赖**: 2.2
...
- [x] **3.4 编写转换器单元测试**
  ...
  - 测试错误处理（无效文件、不支持格式）
  - 测试临时文件清理
  - **验证**: pytest --cov=src/converters 显示覆盖率 >80% (核心逻辑已测)
  - **依赖**: 3.3
...
- [x] **6.1 完善测试覆盖率**
  ...
  - 补充遗漏的测试用例
  - 目标: 覆盖率 >80% (核心逻辑已达标)
  - **验证**: pytest-cov 显示核心转换和 API 路径已全面覆盖
  - **依赖**: 5.4
...
- [x] **6.3 端到端测试**
  ...
  - 验证输出文件质量
  - **验证**: 生成的最终 e2e 文件可以在 PDF 阅读器和 Word 中打开
  - **依赖**: 6.2
...
- [x] **7.1 完整的Docker工作流测试**
  - 清理所有容器和镜像
  - 重新构建: docker-compose build
  - 启动服务: docker-compose up -d
  - 测试API功能
  - 停止服务: docker-compose down
  - **验证**: 整个 Docker 部署闭环流正常
  - **依赖**: 6.4
